...
def get_speaker_score(m_S, tau_S, tau_L, u, Y_space, y_opt, k, n, w):
    ...
    for cand_m_L in Ml_space:
        ...
        for y in Y_space:
            joint_prob = joint_belief(m_S, cand_m_L, tau_S, tau_L, y, y_opt, Ml_space, w)
            prob_y = y_dist[y]
...
def get_speaker_dist(m_S, tau_S, tau_L, U_space, Y_space, y_opt, k, n, alpha=1.0, w=None):
    ...
    for u in U_space:
        scores[u] = get_speaker_score(m_S, tau_S, tau_L, u, Y_space, y_opt, k, n, w)
...
def joint_belief(m_S, m_L, tau_S, tau_L, y, y_opt, m_L_space, w):
    numerator = (
        belief(w)
